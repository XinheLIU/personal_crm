...
```bash
bin/test-lane 2>&1 | tee .gstack/tmp/ship_tests.txt &
npm run test 2>&1 | tee .gstack/tmp/ship_vitest.txt &
wait
```
...
```bash
EVAL_JUDGE_TIER=full EVAL_VERBOSE=1 bin/test-lane --eval test/evals/<suite>_eval_test.rb 2>&1 | tee .gstack/tmp/ship_evals.txt
```
...
```bash
TMPERR_ADV=$(mktemp .gstack/tmp/codex-adv-XXXXXXXX)
...
```bash
TMPERR=$(mktemp .gstack/tmp/codex-review-XXXXXXXX)
